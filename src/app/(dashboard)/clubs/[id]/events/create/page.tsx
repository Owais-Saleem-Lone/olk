...
export default function CreateClubEventPage() {
  ...
  if (loading) return <p className="text-slate-600 text-center py-20">Loading...</p>
  ...
  if (!isCreator) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-sm text-slate-600 mb-6">Ask the organizer of &quot;{clubName}&quot; to create this event.</p>
        <Link href={`/clubs/${clubId}`} className="text-sm text-brand-teal-dark hover:text-teal-700">← Back to {clubName || 'club'}</Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <p className="text-slate-600 mb-8">Schedule a meetup for &quot;{clubName}&quot;</p>
      ...
      <div className="bg-white border border-black/5 rounded-2xl p-8">
        <form onSubmit={handleCreate} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Starts</label>
              <input type="datetime-local" required value={startsAt} onChange={e => setStartsAt(e.target.value)}
                className="w-full bg-slate-100 border border-slate-200 rounded-lg px-3 py-3 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-brand-teal" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Ends <span className="text-slate-500 font-normal">(optional)</span></label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Who can RSVP?</label>
            <select value={visibility} onChange={e => setVisibility(e.target.value as 'public' | 'members_only')}
              ...
              <option value="public" className="bg-white">Anyone (public event)</option>
              <option value="members_only" className="bg-white">Club members only</option>
            </select>
