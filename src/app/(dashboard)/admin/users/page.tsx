    ...
    <div>
      {actionMsg && (
        <div className="mb-4 bg-brand-teal/10 border border-brand-teal/20 text-brand-teal-dark text-sm px-4 py-2 rounded-lg flex justify-between">
          {actionMsg}
          <button onClick={() => setActionMsg('')} className="text-brand-teal-dark/50 hover:text-brand-teal-dark">×</button>
        </div>
      )}
            ...
            filteredUsers.map(u => (
              <button
                ...
                className={`w-full text-left bg-white border rounded-xl p-3 transition-colors ${
                  selected?.id === u.id ? 'border-brand-teal/30 bg-brand-teal/5' : 'border-black/5 hover:bg-slate-50'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-900 truncate">
                      ...
                      {u.is_banned && <span className="ml-2 text-xs text-red-600 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded-full">Banned</span>}
                      {u.is_admin && <span className="ml-2 text-xs text-amber-600 bg-amber-500/10 border border-amber-500/20 px-1.5 py-0.5 rounded-full">{u.admin_role?.replace('_', ' ') || 'Admin'}</span>}
          ...
          <div className="flex-1 min-w-0">
            <div className="bg-white border border-black/5 rounded-2xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-slate-900">{selected.display_name || 'No name'}</h3>
                  <p className="text-xs text-slate-500 font-mono">{selected.id}</p>
                  <p className="text-sm text-slate-600 mt-1">{selected.area_name || 'No area set'}</p>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {selected.is_banned && <span className="text-xs bg-red-500/10 text-red-600 border border-red-500/20 px-2 py-1 rounded-lg">Banned: {selected.ban_reason}</span>}
                {selected.is_admin && <span className="text-xs bg-amber-500/10 text-amber-600 border border-amber-500/20 px-2 py-1 rounded-lg">{selected.admin_role?.replace('_', ' ')}</span>}
              </div>
              ...
              <div className="grid grid-cols-4 gap-2 mb-5">
                {[
                  ...
                  <div key={s.label} className="text-center py-2 bg-white rounded-lg">
              ...
              <div className="flex flex-wrap gap-2 mb-5">
                {selected.is_banned ? (
                  <button onClick={handleUnban} disabled={acting} className="text-xs bg-green-500/10 text-green-600 border border-green-500/20 hover:bg-green-500/20 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">Unban</button>
                ) : (
                  <button onClick={() => setModal('ban')} className="text-xs bg-red-500/10 text-red-600 border border-red-500/20 hover:bg-red-500/20 px-3 py-1.5 rounded-lg transition-colors">Ban</button>
                )}
                ...
                <button onClick={() => handleResetField('display_name')} disabled={acting} className="text-xs bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">Reset Name</button>
                <button onClick={() => handleResetField('area_name')} disabled={acting} className="text-xs bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">Reset Area</button>
                <button onClick={() => setModal('role')} className="text-xs bg-amber-500/10 text-amber-600 border border-amber-500/20 hover:bg-amber-500/20 px-3 py-1.5 rounded-lg transition-colors">Set Role</button>
              </div>
              ...
              {selected.warnings.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm font-medium text-slate-700 mb-2">Warning History</p>
              ...
              {selected.bans.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-slate-700 mb-2">Ban History</p>
                  <div className="space-y-1.5 max-h-40 overflow-y-auto">
                    {selected.bans.map(b => (
                      <div key={b.id} className={`border rounded-lg p-2 ${b.unbanned_at ? 'bg-white border-black/5' : 'bg-red-500/5 border-red-500/10'}`}>
      ...
      {modal === 'ban' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Ban User</h3>
            <p className="text-sm text-slate-600 mb-4">Banning: {selected?.display_name}</p>
            <textarea
              value={banReason}
              onChange={e => setBanReason(e.target.value)}
              placeholder="Reason for ban..."
              rows={3}
              className="w-full bg-slate-100 border border-slate-200 rounded-lg px-4 py-2.5 text-slate-900 text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-red-500 resize-none mb-3"
            />
            <label className="flex items-center gap-2 mb-3">
              <input type="checkbox" checked={banPermanent} onChange={e => setBanPermanent(e.target.checked)} className="rounded" />
              <span className="text-sm text-slate-700">Permanent ban</span>
            </label>
            {!banPermanent && (
              <div className="mb-4">
                ...
                <input
                  ...
                  min={1}
                  className="w-full bg-slate-100 border border-slate-200 rounded-lg px-4 py-2 text-slate-900 text-sm mt-1 focus:outline-none focus:ring-2 focus:ring-red-500"
                />
              </div>
            )}
            <div className="flex gap-2">
              <button onClick={() => setModal(null)} className="flex-1 bg-slate-50 text-slate-600 py-2 rounded-lg text-sm hover:bg-slate-100 transition-colors">Cancel</button>
      ...
      {modal === 'warn' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Warn User</h3>
            <p className="text-sm text-slate-600 mb-4">Warning: {selected?.display_name}</p>
            <textarea
              value={warnReason}
              onChange={e => setWarnReason(e.target.value)}
              placeholder="Reason for warning..."
              rows={3}
              className="w-full bg-slate-100 border border-slate-200 rounded-lg px-4 py-2.5 text-slate-900 text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none mb-4"
            />
            <div className="flex gap-2">
              <button onClick={() => setModal(null)} className="flex-1 bg-slate-50 text-slate-600 py-2 rounded-lg text-sm hover:bg-slate-100 transition-colors">Cancel</button>
              <button onClick={handleWarn} disabled={!warnReason.trim() || acting} className="flex-1 bg-orange-500 text-slate-900 py-2 rounded-lg text-sm hover:bg-orange-400 disabled:opacity-50 transition-colors">
      ...
      {modal === 'role' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setModal(null)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Set Admin Role</h3>
            <p className="text-sm text-slate-600 mb-4">User: {selected?.display_name}</p>
            <select
              value={newRole}
              onChange={e => setNewRole(e.target.value)}
              className="w-full bg-slate-100 border border-slate-200 rounded-lg px-4 py-2.5 text-slate-900 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-amber-500"
            >
