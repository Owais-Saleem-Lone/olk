"use client"

import { useState, useEffect } from 'react'
import Image from 'next/image'
import { createPortal } from 'react-dom'

type BookOfMonth = {
  title: string
  author: string | null
  description: string | null
  cover_url: string | null
  month_label: string | null
}

const BookIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-600">
    <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/>
    <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
  </svg>
)

export default function BookOfMonthCard({ book }: { book: BookOfMonth }) {
  const [open, setOpen] = useState(false)

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setOpen(false) }
    if (open) {
      document.addEventListener('keydown', onKey)
      document.body.style.overflow = 'hidden'
    }
    return () => {
      document.removeEventListener('keydown', onKey)
      document.body.style.overflow = ''
    }
  }, [open])

  return (
    <>
      {/* ── Preview Card ── */}
      <div className="relative bg-gradient-to-br from-amber-50 via-white to-teal-50/40 border border-amber-200 rounded-3xl p-8 overflow-hidden shadow-sm">
        <div className="pointer-events-none absolute -top-10 -right-10 w-64 h-64 bg-amber-200/40 rounded-full blur-3xl" />

        <div className="relative flex gap-8 items-start">
          {/* Cover */}
          <div className="relative flex-shrink-0 w-36 aspect-[2/3] rounded-xl overflow-hidden border border-black/5 shadow-xl shadow-amber-900/10">
            {book.cover_url ? (
              <Image src={book.cover_url} alt={book.title} fill unoptimized sizes="144px" className="object-cover" />
            ) : (
              <div className="w-full h-full bg-gradient-to-br from-amber-100 to-slate-100 flex items-center justify-center">
                <BookIcon />
              </div>
            )}
          </div>

          {/* Info */}
          <div className="flex-1 min-w-0 py-1">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-amber-600 text-xs font-bold uppercase tracking-widest">✦ Book of the Month</span>
              {book.month_label && (
                <span className="text-xs text-slate-600">— {book.month_label}</span>
              )}
            </div>

            <h3 className="text-2xl font-bold text-slate-900 leading-tight mb-1">{book.title}</h3>
            {book.author && (
              <p className="text-slate-500 text-sm mb-4">by {book.author}</p>
            )}
            {book.description && (
              <p className="text-slate-400 text-sm leading-relaxed line-clamp-3 mb-5">
                {book.description}
              </p>
            )}

            <button
              onClick={() => setOpen(true)}
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-600 hover:text-amber-500 transition-colors group"
            >
              Read More
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="group-hover:translate-x-0.5 transition-transform">
                <path d="M5 12h14"/><path d="m12 5 7 7-7 7"/>
              </svg>
            </button>
          </div>
        </div>
      </div>

      {/* ── Modal (portal so it escapes all z-index stacking contexts) ── */}
      {open && createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4"
          onClick={() => setOpen(false)}
        >
          {/* Backdrop */}
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md" />

          {/* Panel — outer glow ring */}
          <div
            className="relative max-w-lg w-full rounded-3xl shadow-[0_0_80px_-8px_rgba(245,158,11,0.35)]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Gradient border */}
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-amber-300 via-amber-200 to-teal-200 p-px">
              <div className="w-full h-full rounded-3xl bg-white" />
            </div>

            {/* Content area */}
            <div className="relative rounded-3xl bg-gradient-to-br from-amber-50 via-white to-teal-50/30 p-8 overflow-hidden">
              {/* Inner ambient glow */}
              <div className="pointer-events-none absolute top-0 left-1/2 -translate-x-1/2 w-72 h-28 bg-amber-200/30 blur-3xl rounded-full" />

              {/* Close button */}
              <button
                onClick={() => setOpen(false)}
                className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-amber-100 hover:bg-amber-200 border border-amber-200 text-amber-700 hover:text-amber-800 transition-colors text-sm"
              >
                ✕
              </button>

              {/* Cover + title row */}
              <div className="relative flex gap-6 mb-6">
                <div className="relative flex-shrink-0 w-28 aspect-[2/3] rounded-xl overflow-hidden border border-black/5 shadow-xl shadow-amber-900/10">
                  {book.cover_url ? (
                    <Image src={book.cover_url} alt={book.title} fill unoptimized sizes="112px" className="object-cover" />
                  ) : (
                    <div className="w-full h-full bg-gradient-to-br from-amber-100 to-slate-100 flex items-center justify-center">
                      <BookIcon />
                    </div>
                  )}
                </div>

                <div className="flex-1 min-w-0 py-1">
                  <span className="text-amber-600 text-xs font-bold uppercase tracking-widest">✦ Book of the Month</span>
                  {book.month_label && (
                    <p className="text-amber-600/70 text-xs mt-0.5 mb-3">{book.month_label}</p>
                  )}
                  <h2 className="text-xl font-bold text-slate-900 leading-tight mb-1">{book.title}</h2>
                  {book.author && (
                    <p className="text-slate-500 text-sm">by {book.author}</p>
                  )}
                </div>
              </div>

              {/* Divider */}
              <div className="relative border-t border-amber-900/10 mb-5" />

              {/* Description */}
              {book.description ? (
                <p className="relative text-slate-700 text-sm leading-relaxed">{book.description}</p>
              ) : (
                <p className="relative text-slate-600 text-sm italic">No description added yet.</p>
              )}
            </div>
          </div>
        </div>,
        document.body
      )}
    </>
  )
}
