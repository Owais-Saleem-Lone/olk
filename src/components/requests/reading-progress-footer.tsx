    ...
    <div className="border-t border-black/5 pt-3 space-y-3">
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-xs text-slate-600">📖 Reading progress</span>
          <span className="text-xs text-brand-teal-dark font-semibold tabular-nums">
            {progress}%
          </span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={saving}
            className="text-xs bg-brand-teal/10 text-brand-teal-dark border border-brand-teal/20 hover:bg-brand-teal/20 disabled:opacity-50 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
          ...
          {isConfirmingComplete ? (
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-xs text-slate-700 flex-1">
                The book will move to <span className="text-slate-900 font-medium">your My Books</span> as available — ready to pass on to the next reader.
              </p>
              <button
                onClick={onComplete}
                disabled={completing}
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="text-xs bg-brand-teal/20 hover:bg-brand-teal/30 text-teal-700 border border-brand-teal/30 font-semibold px-4 py-1.5 rounded-lg transition-colors whitespace-nowrap"
              >
                Complete Reading
              </button>
            </div>
      ...
      {progress === 100 && req.books?.listing_type === 'lend' && (
        <p className="text-xs text-blue-600 bg-blue-500/[0.05] border border-blue-500/20 rounded-xl px-4 py-3">
