...
These six steps are still exactly what happens, but as of a code-quality pass, steps 1, 5, and 6 are no longer written inside \code{banUser} itself — \code{banUser} is now a handler passed to a shared \code{withAdminAction('moderator', 'ban\_user', handler)} wrapper that performs the guard \emph{before} calling the handler and the log/revalidate \emph{after} it returns, identically for all 24 admin actions (Chapter~\ref{ch:server-actions} has the full code). Steps 2–4 above are the part that's genuinely specific to banning someone, and are all that's left inside \code{banUser}'s own handler body.
