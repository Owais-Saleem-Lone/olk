...
The config (\pathname{eslint.config.mjs}) is Next.js's flat-config defaults (\code{core-web-vitals} + \code{typescript}) with no project-specific rules added. \pathname{.github/workflows/ci.yml} runs \code{npm run lint} and \code{npm test} on every push and pull request against \code{main}, and both pass cleanly.
