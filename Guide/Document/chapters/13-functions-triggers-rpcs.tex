...
\code{get\_clubs\_nearby(user\_lat, user\_lng)} is the same haversine pattern applied to \code{clubs} (only \code{active = true}), joined to the creator's profile, ordered by distance then by \code{member\_count DESC}.
...
\section{Club membership counting}
