...
\begin{lstlisting}[style=olkcodesql, caption={get\_books\_nearby — haversine distance, nulls last, bounded}]
CREATE OR REPLACE FUNCTION public.get_books_nearby(
  user_lat double precision, user_lng double precision, p_limit integer DEFAULT 200
)
RETURNS TABLE(id uuid, title varchar, author varchar, condition varchar,
              ...
              acquired_via_donation boolean, description text, publication_year smallint)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT
    ...
    b.read_count, b.acquired_via_donation, b.description, b.publication_year
  FROM books b
...
\begin{olkwarn}
\code{p\_limit} bounds the \emph{result size}, not the \emph{work}. Postgres still computes \code{distance\_km} for every row matching the \code{status} filter and sorts all of them before applying \code{LIMIT} — there is no index that can help here, since the sort key is a computed expression, not a stored column. At today's catalog size this is cheap; it will not stay cheap as the \code{books} table grows toward the scale discussed in Chapter~\ref{ch:welcome}'s scaling roadmap. A spatial index or a coarser area-bucket pre-filter is the real fix, not yet done — see \S\ref{sec:phase1} of that chapter.
\end{olkwarn}
...
\begin{olkgotcha}
Adding \code{p\_limit} above (23-07-2026, Chapter~\ref{ch:welcome}'s scaling pass) hit a second, independent Postgres surprise: the first attempt used a plain \code{CREATE OR REPLACE FUNCTION} with the new third parameter appended, on the theory that a default value would keep every existing 2-argument call site working unchanged. It compiled and applied without error — and did nothing. Postgres identifies a function by its name \emph{and} its parameter type list together; adding a parameter changes that identity, so \code{CREATE OR REPLACE} registered a brand-new 3-argument \emph{overload} sitting \emph{alongside} the untouched original 2-argument function, which still had no \code{LIMIT}. Since \pathname{browse/page.tsx} calls \code{.rpc('get\_books\_nearby', \{ user\_lat, user\_lng \})} with exactly two arguments, Postgres kept resolving every real call to the old, unfixed overload — confirmed by running \code{\textbackslash df public.get\_books\_nearby} against a freshly-reset local database (Chapter~\ref{ch:local-env}) and finding two rows, not one, where only one was expected. The fix is the explicit \code{DROP FUNCTION IF EXISTS public.get\_books\_nearby(double precision, double precision);} that now appears at the top of \pathname{20260723143629\_bound\_get\_books\_nearby\_limit.sql}, before the \code{CREATE OR REPLACE}. \textbf{Lesson:} \code{CREATE OR REPLACE FUNCTION} only replaces a function with the \emph{exact same} parameter type list. Changing the return type (the gotcha above) and changing the parameter list (this one) are two separate reasons the same fix — drop the old signature explicitly first — is needed; after any signature change, verify with \code{\textbackslash df} that exactly one overload exists before trusting the fix.
\end{olkgotcha}
...
\begin{olknote}
\code{update\_club\_member\_count()} grew a third branch, \code{TG\_OP = 'UPDATE'}, once membership became request-then-approve (Chapter~\ref{ch:wishlists-clubs}) rather than instant. A plain \code{INSERT} only increments when the new row already lands as \code{status = 'approved'} (true only for a club's own creator, inserted directly by \code{approve\_club\_request} — everyone else's join request lands \code{pending} and doesn't move the count yet). An \code{UPDATE} increments when \code{OLD.status} transitions \emph{into} \code{'approved'} and decrements on the reverse transition; a \code{DELETE} decrements only if the row being removed was already \code{approved} — so withdrawing or rejecting a still-pending request is correctly a no-op for the count, exactly as if the request had never been filed.
\end{olknote}

\code{is\_club\_member(club\_id, user\_id)} and \code{is\_event\_participant(event\_id, user\_id)}, both \code{STABLE SECURITY DEFINER SQL} functions, are the single source of truth for "is this user really in this club" / "did this user really RSVP to this event" — every policy and trigger that needs that answer (the \code{club\_members}/\code{event\_rsvps} SELECT policies, the \code{club\_posts}/\code{club\_ratings} INSERT policies, the \code{event\_rsvps} members-only RSVP check) calls one of these two rather than re-deriving the check inline. \code{is\_club\_member} specifically only counts \code{status = 'approved'} rows — a pending applicant is not, yet, a member for any of these purposes. See Chapter~\ref{ch:rls}'s gotcha on why these have to be \code{SECURITY DEFINER}: a plain \code{EXISTS} subquery against the policy's own table causes Postgres to reject the policy outright with an infinite-recursion error, a sharper failure mode than the usual "just returns the wrong rows" RLS mistake.

\section{Club chat, ratings, and per-club/per-event limits}

\code{enforce\_club\_post\_rate\_limit()}, trigger \code{trg\_enforce\_club\_post\_rate\_limit} (\code{BEFORE INSERT ON club\_posts}), is the same shape as \code{enforce\_message\_rate\_limit} (below) but role-aware: it looks up whether \code{NEW.author\_id} is that club's \code{creator\_id} and applies \code{club\_chat\_owner\_messages\_per\_hour} (30) if so, \code{club\_chat\_member\_messages\_per\_hour} (10) otherwise, both read through \code{get\_platform\_setting\_int} — counted across all of that author's posts in the last hour, not scoped per-club, the same "global per user" shape \code{enforce\_message\_rate\_limit} already uses for \code{messages}.

\code{enforce\_club\_event\_monthly\_limit()}, trigger \code{trg\_enforce\_club\_event\_monthly\_limit} (\code{BEFORE INSERT ON club\_events}), counts \code{club\_events} rows for \code{NEW.club\_id} created in the trailing 30 days (by \code{created\_at}, regardless of the counted rows' own \code{active} flag) against \code{platform\_settings.max\_club\_events\_per\_month} (1), raising \code{RATE\_LIMIT\_EXCEEDED} once the club has used its slot for the window.

\code{enforce\_club\_event\_capacity\_cap()}, trigger \code{trg\_enforce\_club\_event\_capacity\_cap} (\code{BEFORE INSERT OR UPDATE ON club\_events}), rewrites \code{NEW.capacity} rather than merely validating it: a \code{NULL} capacity is set \emph{to} \code{platform\_settings.max\_event\_capacity} (10) instead of being left as "unlimited," and a capacity above the cap is rejected outright with the same \code{RATE\_LIMIT\_EXCEEDED} error code the rate-limit triggers use, so the client-side error handling in \pathname{clubs/[id]/events/create/page.tsx} can treat all of these limits identically.

\code{update\_club\_rating\_stats()}, trigger \code{trg\_club\_rating\_stats} (\code{AFTER INSERT OR UPDATE OR DELETE ON club\_ratings}), recomputes \code{clubs.rating\_avg}/\code{rating\_count} for the affected club from a full \code{avg}/\code{count} over \code{club\_ratings} on every change, rather than incrementally adjusting a running sum the way \code{update\_club\_member\_count} does — ratings can be edited in place (a member re-rating a club just \code{UPSERT}s their existing row), so a full recompute sidesteps having to separately track "subtract the old score, add the new one" on every update.

\begin{olknote}
The two bulk notification RPCs, \code{notify\_club\_chat\_message(post\_id)} and \code{notify\_event\_created(event\_id)}, replace what used to be a client-side loop calling \code{createNotification} once per club member — see Chapter~\ref{ch:server-actions} for why that loop was a real scaling risk, and how these two \code{SECURITY DEFINER} functions collapse it into a single \code{INSERT ... SELECT ... RETURNING} each.
\end{olknote}
...
\code{mark\_club\_request\_identity\_verified(request\_id)}, \code{SECURITY DEFINER}, \code{moderator}-gated: flips \code{club\_requests.identity\_verified} to \code{true} and stamps \code{identity\_verified\_at}/\code{identity\_verified\_by}, only while the request is still \code{pending}. The actual ID/CV exchange happens over email, entirely outside the app — this function only records that an admin did it. \code{approve\_club\_request} (above) now checks \code{v\_request.identity\_verified} before doing anything else and raises an exception if it's still \code{false}, so a request can never become a real club without a moderator explicitly having cleared this gate — a hard stop in the database, not just a disabled button in \pathname{admin/club-requests/page.tsx} (Chapter~\ref{ch:wishlists-clubs}'s recurring "is it a real gate" theme, applied one feature further).
