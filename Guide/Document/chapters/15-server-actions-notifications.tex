...
\section{Fan-out notifications: one RPC, not one round trip per member}

\code{club\_join} notifies a single person (the owner, when someone requests to join) and \code{club\_membership\_approved} notifies a single person back (the applicant, once approved) — both a single \code{createNotification} call, no different from notifying someone their book request was accepted. Broadcasting to an entire club — a new chat message, a newly created event — is a different shape: potentially dozens of recipients from one action, and it used to be exactly that many sequential \code{createNotification} calls, each one its own \code{can\_notify} round trip, its own \code{INSERT}, and its own \emph{blocking} \code{await sendNotificationEmail(...)} before the loop moved to the next member. Posting one message to a 200-member club meant the poster's browser sat waiting through 200 sequential server round trips, any one of which failing partway through silently stopped notifying the rest — and on a serverless function with a wall-clock timeout, a large enough club could fail the whole request outright.

\code{notifyClubChatMessage(postId)} and \code{notifyEventCreated(eventId)} (Chapter~\ref{ch:pages} call sites: \pathname{clubs/[id]/page.tsx}'s \code{handlePost}, \pathname{clubs/[id]/events/create/page.tsx}'s \code{handleCreate}) replace that loop with a single call each to \code{notify\_club\_chat\_message}/\code{notify\_event\_created} (Chapter~\ref{ch:functions}) — one \code{INSERT ... SELECT ... RETURNING} that authorizes the caller and inserts every recipient's notification row in one statement, returning the \code{(user\_id, title)} pairs it just created. Round-trip count for the DB-visible half of the fan-out is now $O(1)$ regardless of club size.

\begin{lstlisting}[caption={the shape of both, from src/lib/notifications.ts}]
export async function notifyClubChatMessage(postId: string) {
  ...
  const items = (recipients ?? []).map(r => ({ userId: r.user_id, type: 'club_announcement', title: r.title }))
  after(() => sendBatchNotificationEmails(items))
}
\end{lstlisting}

The email half still can't be a single round trip — Resend sends one email per recipient — so it's handled two ways instead: \code{after()}, imported from \code{next/server}, defers the whole email batch until \emph{after} the response has already gone back to the poster/organizer, so they're never blocked waiting on Resend at all — the same "verify against \pathname{node\_modules/next/dist/docs} rather than assumed" habit this fork's docs-first quirks already teach elsewhere (Chapter~\ref{ch:proxy}), since \code{after()}'s exact semantics (it extends the current invocation, it isn't a queue) are easy to get wrong from memory. \code{sendBatchNotificationEmails} (Chapter~\ref{ch:functions}) then sends that batch in chunks of 10 concurrent requests via \code{Promise.allSettled}, so one slow or failed send can't stall — or silently swallow — the rest of the batch, and a club with hundreds of members doesn't fire hundreds of simultaneous outbound requests at once.

\begin{olkgotcha}
\code{after()} only extends the life of the \emph{current} serverless invocation — it is not a durable queue. If the platform kills the function before the deferred callback finishes (a very large batch, or a platform outage), those emails are simply lost; the in-app notification row, already committed before \code{after()} was even scheduled, is not. This is the same "in-app row is the source of truth, email is best-effort" posture the rest of this chapter already takes for single-recipient notifications, just applied to a batch instead of one send. Revisit this if club sizes ever grow to the point where "batches of 10, best-effort" stops being enough — a real job queue would be the next step, not a bigger batch size.
\end{olkgotcha}
...
\begin{enumerate}
  ...
  \item Decide which existing \code{context.kind} the new call site's relationship maps to (\code{'request'}, \code{'club\_join'}, \code{'club\_membership\_approved'}, \code{'club\_announcement'}, \code{'event\_created'}, or \code{'wishlist\_match'}) — or, if it's a genuinely new kind of relationship, add a new branch to \code{can\_notify()} (Chapter~\ref{ch:rls}) \emph{before} wiring up the call site. \code{'event\_created'} itself is the \code{'club\_announcement'} shape one table over: the caller must be the event's own \code{creator\_id}, and the target must be a real member of that event's club. \code{'club\_membership\_approved'} is \code{'club\_join'} reversed — the caller must be the club's \code{creator\_id}, and the target must now actually have an \code{approved} \code{club\_members} row. \code{createNotification} throws \code{"Forbidden"} if \code{can\_notify} doesn't recognise the \code{context.kind} or can't verify the relationship, so a new notification type with no matching branch fails loudly for every caller rather than silently succeeding.
\end{enumerate}
\end{olkhowto}
