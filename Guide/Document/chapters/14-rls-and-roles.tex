...
\codecell{clubs} & Anyone (incl. anon) SELECTs active clubs — description, cover, interests, and the denormalized \codecell{rating\_avg}/\codecell{rating\_count} are all public; there is no regular-user INSERT policy at all (creation only happens through \codecell{approve\_club\_request}, Chapter~\ref{ch:wishlists-clubs}); the creator UPDATE/DELETEs their own; admins/mods UPDATE any, only \codecell{super\_admin} DELETEs any. \\
\codecell{club\_members} & SELECT is member/owner-scoped, \emph{not} open to every authenticated user: a user always sees their own row, an \codecell{approved} member sees the rest of that club's \codecell{approved} roster, and the club's creator (or an admin/mod) sees every row including \codecell{pending} applications. A user INSERTs themself as \codecell{status = 'pending'} only — \codecell{WITH CHECK} rejects any insert that tries to set \codecell{'approved'} directly, closing off self-approval. Only the club's creator can UPDATE a row, and only to \codecell{'approved'} (the \codecell{WITH CHECK} pins the destination value). A user DELETEs their own row (leave, or withdraw a pending request); the creator or an admin/mod can remove anyone. \\
\codecell{club\_posts} & Anyone SELECTs (club chat content stays as public as the club itself); any \emph{approved} member INSERTs (posts) — not creator-only anymore; the post's own author, the club's creator, or an admin/mod can DELETE it. No UPDATE at all. \\
\codecell{club\_ratings} & Authenticated users SELECT all rows (rater identity + comment; the public-facing summary is \codecell{clubs.rating\_avg}/\codecell{rating\_count} instead); an \emph{approved} member INSERTs/UPDATEs only their own rating; the rater or an admin/mod DELETEs it. \\
...
\codecell{event\_rsvps} & SELECT is member/participant-scoped, not open to every authenticated user: a user always sees their own RSVP, any \emph{approved} club member sees the full attendee list for that club's events, and any other participant of that \emph{same} event sees the rest of its attendees too (so a non-member who RSVP'd to a public event isn't locked out of seeing who else is going) — plus admins/mods. A user INSERTs themself, but only if the event is \codecell{public} \emph{or} they're an approved club member, \emph{and} it isn't already at \codecell{capacity}; a user DELETEs only their own RSVP. \\
...
\section{Seven RLS looseness gotchas — since tightened}
...
\begin{olknote}
\textbf{\code{club\_members}/\code{event\_rsvps} SELECT — open to every authenticated user, not just the people it actually concerns.} Both policies used to read simply \code{USING (true)} for any \code{authenticated} caller: a club's full membership roster, and any event's full attendee list, were readable by anyone with an account, regardless of whether they belonged to that club or event at all. Clubs and events themselves are deliberately public (Chapter~\ref{ch:wishlists-clubs}) — but \emph{who specifically} is in them was never meant to be, and nothing about "the club is discoverable" implies "its roster should be too." Both policies now require the caller to actually be an approved member (via \code{is\_club\_member()} below) or, for \code{event\_rsvps}, a participant of that specific event, with the club's creator and admins/mods always able to see everything in their own club.
\end{olknote}

\begin{olkgotcha}
\textbf{Fixing the gotcha above the naive way breaks Postgres outright.} The first attempt at the \code{club\_members} SELECT policy wrote the membership check as a plain \code{EXISTS} subquery against \code{club\_members} itself — the obvious, direct translation of "does a row exist showing I'm in this same club." Every existing RLS pattern in this schema queries a \emph{different} table from inside a policy (Chapter~\ref{ch:wishlists-clubs}'s \code{club\_posts} INSERT checking \code{clubs}, for instance) without incident, so this looked like the same shape. It isn't: Postgres statically rejects a policy that queries its own table with \code{ERROR: infinite recursion detected in policy for relation "club\_members"} (\code{42P17}) the moment anything else needs to evaluate that policy while scanning the table — caught immediately by the existing \code{tests/rls/event-rsvp-capacity.test.ts} suite (Chapter~\ref{ch:workflow}), whose RSVP capacity check joins into \code{club\_members} and tripped the cycle on the very next test run. This holds even though the underlying logic obviously terminates (one branch is a direct \code{auth.uid()} equality) — it's a structural limitation of how Postgres unfolds RLS policies, not a bug in the logic itself. The fix: route the self-check through a \code{STABLE SECURITY DEFINER} function, \code{is\_club\_member(club\_id, user\_id)} (and its \code{event\_rsvps} counterpart, \code{is\_event\_participant}) — owned by \code{postgres}, so its internal \code{SELECT} runs as the table owner and never re-enters RLS on the same table at all. Same idiom Supabase's own multi-tenancy docs recommend for exactly this "is this user in the same group as this row" shape; reach for it by default any time a policy would otherwise reference its own table.
\end{olkgotcha}
