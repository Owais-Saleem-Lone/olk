...
The main discovery surface: search, genre/type/condition/area filters, a distance radius slider, request/bookmark actions, and entry points into the notes and report modals. It calls the \code{get\_books\_nearby} RPC (Chapter~\ref{ch:functions}) when the viewer has a location set, and falls back to a plain ordered query otherwise — bounded to 200 rows either way (Chapter~\ref{ch:welcome}'s scaling roadmap).

\begin{olknote}
The plain-query fallback branches again on whether a search or filter is active. With none active, the page fetches from \pathname{/api/books} (Chapter~\ref{ch:proxy}) — a cached, shared response, since that exact result is identical for every visitor. The moment a search term or a genre/type/condition filter is set, it goes back to querying Supabase directly, uncached, because a filtered result isn't the same for every visitor and caching it under a shared key would be wrong. This split — not "always cache" or "never cache" — is deliberate; see Chapter~\ref{ch:architecture}'s note on the same page for the general rule it follows.
\end{olknote}
...
\section{Clubs \& Events — directory, creation, and detail}
...
Club detail (\pathname{clubs/[id]/page.tsx}) fans out a notification to every member when a new post is created, routed one at a time through the \code{createNotification} server action (rather than a bulk client-side \code{.insert(notifications[])}, now that direct cross-user notification inserts are blocked by RLS — Chapter~\ref{ch:rls}). Its "delete club" is a soft-delete (\code{active: false}), confirmed through the shared \code{ConfirmModal} component like the rest of the app's destructive actions. It also lists the club's upcoming events (Chapter~\ref{ch:wishlists-clubs}) and, for the creator, a "Create Event" link into \pathname{clubs/[id]/events/create/page.tsx}, which fans out its own \code{event\_created} notification the same way.

The cross-club \pathname{events/page.tsx} directory and \pathname{events/[id]/page.tsx} detail page follow the same shape as their club equivalents — geo-sorted list with a plain-query fallback, RSVP as a toggleable insert/delete on \code{event\_rsvps} — with one addition: an "Add to calendar" link straight to the \pathname{.ics} Route Handler (Chapter~\ref{ch:server-actions}), no client-side calendar library involved.

\section{The admin section}

Chapter~\ref{ch:admin} is a full end-to-end treatment of the admin/moderation subsystem; this section is a quick map of the ten admin pages so you know where to look.
