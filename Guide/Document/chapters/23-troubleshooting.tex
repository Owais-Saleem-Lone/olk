...
\end{lstlisting}
Count the tables against Chapter~\ref{ch:schema} (27, as of this edition). Fewer than that means the volume predates recent migrations — run \code{npx supabase db reset} to bring it current. Chapter~\ref{ch:local-env} documents a real instance of this happening.
...
\section*{"A migration applies fine, but the fix doesn't seem to actually work"}
Before assuming the fix itself is wrong, check whether you changed a function's \emph{parameter} list without dropping the old signature first — \code{CREATE OR REPLACE FUNCTION} silently creates a new overload instead of replacing the function when the parameter types differ, even with a default value on the new parameter (Chapter~\ref{ch:functions}'s second \code{get\_books\_nearby} gotcha is a real instance of exactly this). Run \code{\textbackslash df public.your\_function\_name} in Studio's SQL editor or via \code{psql} — more than one row means more than one overload exists, and PostgREST/your old call sites are very possibly still hitting the stale one.
...
\section*{"Sentry issues show 40+ undifferentiated stack frames and my own code isn't highlighted"}
Not something wrong with your code specifically — Turbopack emits server/edge stack frames for app code as bare relative paths (\code{src/app/.../route.ts}), which both the Sentry Node SDK's and Sentry's backend's "is this a Node internal" heuristics misread as framework code, marking it \code{in\_app: false} the same as a real \pathname{node\_modules} frame. \pathname{src/instrumentation.ts}'s \code{beforeSend} hook corrects this by rewriting the path to look absolute before the event is sent (Chapter~\ref{ch:envvars}, Chapter~\ref{ch:playbook}'s Sentry-triage entry). If you see this again after touching that file, check the hook is still running and still matching the actual filename shape Turbopack is currently emitting — a Sentry SDK upgrade or a Next.js bundler change could shift it, and the only reliable way to confirm is the same one used to find this bug originally: trigger a real error and check \code{in\_app} on the actual event via the API, not by assumption.

\section*{"I'm not sure if something is really blocked by RLS or just by the UI"}
This distinction matters enough that it recurs as its own exercise in Chapters~\ref{ch:pages},~\ref{ch:wishlists-clubs}, and~\ref{ch:admin}.

\begin{olkhowto}
The reliable test: open the browser devtools network tab (or Studio's SQL editor with \code{SET ROLE authenticated;} / \code{SET ROLE anon;}), and try the operation directly against the REST API or SQL editor rather than through the app's UI. If it succeeds there despite the UI normally preventing it, the gate is UI-only.
\end{olkhowto}
