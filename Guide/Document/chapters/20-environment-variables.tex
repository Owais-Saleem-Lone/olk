...
\codecell{NEXT\_PUBLIC\_SENTRY\_DSN} & No & Read in \pathcell{src/instrumentation.ts} and \pathcell{src/instrumentation-client.ts}. If unset, Sentry safely no-ops (same convention as \codecell{RESEND\_API\_KEY}) — no errors are captured, but nothing breaks. Set it to enable production error monitoring. \\
\codecell{SENTRY\_AUTH\_TOKEN} & No & Build-time only, read in \pathcell{next.config.ts}. Enables source-map upload so Sentry shows readable stack traces instead of minified ones. Without it, \codecell{withSentryConfig} skips source-map upload entirely rather than failing the build. \\
\codecell{SENTRY\_ORG}, \codecell{SENTRY\_PROJECT} & No & Build-time only, paired with \codecell{SENTRY\_AUTH\_TOKEN} for source-map upload. Both come from the Sentry project's settings page. \\
\bottomrule
\end{longtable}
\end{center}
...
\codecell{NEXT\_PUBLIC\_SENTRY\_DSN} & Usually omitted (errors just aren't captured locally) & Set, so production errors are visible \\
\bottomrule
\end{longtable}
\end{center}
...
\begin{olknote}
\code{SENTRY\_AUTH\_TOKEN} is asked to do two different jobs that need two different token types, and conflating them causes a silent failure rather than a loud one. \pathname{next.config.ts} wants a token scoped to \emph{write} — source-map/release upload — which today means an Organization Token (Settings \textrightarrow{} Organization Tokens) created with the \code{org:ci} scope, currently the only scope that page offers. Querying issues back out of Sentry programmatically (Chapter~\ref{ch:playbook}'s "I need to check whether a real error has shown up in Sentry" entry) needs the opposite — \emph{read} access (\code{org:read}, \code{project:read}, \code{event:read}) — which Organization Tokens don't expose at all; that requires a Personal Token (Settings \textrightarrow{} Account \textrightarrow{} API \textrightarrow{} Personal Tokens) instead. If you put a read-only Personal Token in \pathname{.env.local} as \code{SENTRY\_AUTH\_TOKEN} for local API querying, a local production build will then also pick it up for source-map upload and silently skip that step (\code{silent: true} in \pathname{next.config.ts}) since it lacks write scope — harmless, but confusing if you go looking for why source maps didn't upload.
\end{olknote}

\begin{olktask}
Grep the entire \pathname{src/} tree for \code{process.env} (\code{grep -rn "process.env" src/}) and confirm the result set matches exactly the eleven variables in this chapter — no more, no fewer (\code{NEXT\_RUNTIME} is Next.js's own runtime constant, not something a deployer sets, so don't count it). If you ever add a twelfth, this chapter needs a matching new row in the same pull request (Chapter~\ref{ch:workflow}'s rule about keeping this manual in sync applies here too).
\end{olktask}
