...
alter table "public"."book_requests" drop constraint "book_requests_status_check";

alter table "public"."books" drop constraint "books_condition_check";

alter table "public"."books" drop constraint "books_listing_type_check";

alter table "public"."books" drop constraint "books_status_check";

drop index if exists "public"."idx_unique_active_request";

CREATE UNIQUE INDEX idx_unique_active_request ON public.book_requests USING btree (book_id, requester_id) WHERE ((status)::text = ANY ((ARRAY['pending'::character varying, 'accepted'::character varying, 'handed_over'::character varying])::text[]));

alter table "public"."book_requests" add constraint "book_requests_status_check" CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'accepted'::character varying, 'declined'::character varying, 'handed_over'::character varying, 'returned'::character varying])::text[]))) not valid;

alter table "public"."book_requests" validate constraint "book_requests_status_check";

alter table "public"."books" add constraint "books_condition_check" CHECK (((condition)::text = ANY ((ARRAY['excellent'::character varying, 'good'::character varying, 'fair'::character varying, 'poor'::character varying])::text[]))) not valid;

alter table "public"."books" validate constraint "books_condition_check";

alter table "public"."books" add constraint "books_listing_type_check" CHECK (((listing_type)::text = ANY ((ARRAY['donate'::character varying, 'lend'::character varying])::text[]))) not valid;

alter table "public"."books" validate constraint "books_listing_type_check";

alter table "public"."books" add constraint "books_status_check" CHECK (((status)::text = ANY ((ARRAY['available'::character varying, 'unavailable'::character varying, 'given'::character varying])::text[]))) not valid;
